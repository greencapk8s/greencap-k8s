...
**PodState**:
The single label GreenCap displays for a Pod, answering "is this healthy?" rather than "what lifecycle stage is this in?". A derived concept — no single Kubernetes API field carries it. Computed from the Pod's `phase` combined with the waiting reason of its containers and abnormal terminations, because `phase` alone reports whether the Pod was admitted and its containers created, not whether they are actually working: a Pod whose image cannot be pulled, or whose container is crash-looping, keeps reporting `phase: Running`. Deriving the displayed state client-side matches what `kubectl get pods` does — its STATUS column is likewise computed, not read from the API. Distinct from `phase`, which remains in use where the real lifecycle stage is what matters (skipping the log API call for Pods that have no started containers, and identifying successfully completed Job Pods). Its vocabulary is the raw Kubernetes reason, never translated — the value shown is whatever the kubelet reported (`CrashLoopBackOff`, `ImagePullBackOff`, `CreateContainerConfigError`, ...), an open set that grows with Kubernetes rather than a curated list, falling back to the `phase` when no container reports a problem. Raw reasons are kept deliberately even though GreenCap targets beginners: they are what the user can search for and what every tutorial and `kubectl` output uses, so translating would soften the label at the cost of stranding the user in diagnosis. Beginner support comes instead from a tooltip carrying the reason's message, the same affordance already used for a failed Operator's badge. Alongside its label, a PodState carries a severity — healthy, transient, or problem — which is what colors the badge; since the labels form an open set, only the benign side is enumerated (the transient reasons a Pod passes through on its way up or down), and anything unrecognized is treated as a problem, the same fail-closed posture GreenCap takes with access control. When containers disagree, a problem always wins over a healthy aggregate — a crash-looping sidecar is never masked by its Pod's other containers running — and between two problem containers the first in `spec` order wins — stable, and the order the user already sees in the manifest and in the log view's container selector; `kubectl` resolves ties the same way, verified against a live two-container Pod whose first container was terminated with `Error` and whose second could not pull its image, where `kubectl` reported `Error`. A `Running` PodState therefore asserts that no container reported a problem, rather than echoing the `phase` field — a meaningful difference, since `phase` can remain `Running` on a Pod with zero running containers. Restart counts are deliberately excluded from the derivation: a container that restarted many times but is healthy now tells a different story than one broken now, and the listing carries restarts as its own column. Resolved in order, stopping at the first match: a Pod awaiting deletion is `Terminating` (the same `deletionTimestamp` derivation already used for PersistentVolumeClaim and PersistentVolume, and outranking everything else — a container dying inside a Pod the user deleted is expected, not news); then a troubled init container, carried with `kubectl`'s `Init:` prefix since a failure there means the main containers never started at all; then a troubled regular container; then `ContainersNotReady` when the Pod's `Ready` condition is false for that reason — the case of containers that run but never become ready, which Kubernetes reports only through that condition (a failing probe surfaces as an Event, never as a container state), and which is keyed on the condition's own reason rather than on the phase, since Kubernetes already distinguishes it from the `PodCompleted` reason carried by finished Pods; and finally, when nothing reports a problem, the `phase` itself. A container counts as troubled when it is waiting with any reason, or terminated with a non-zero exit code — the exit code matters, otherwise a container that finished successfully would steal the label from a legitimately running sibling. Two consequences are accepted deliberately: `ContainerCreating` becomes briefly visible on every new deployment (it is the honest raw reason, and what `kubectl` shows), and a completed Job Pod still reads `Succeeded` rather than `kubectl`'s `Completed`, because it falls through to the phase and `Succeeded` is already the documented vocabulary keying the listing's hide-completed-Job-Pods toggle. The unready case is spelled out in full as `ContainersNotReady` rather than shortened to `NotReady`, which the glossary already spends on Node and which would leave the user unsure whether the Pod or its Node is unready. It is also where GreenCap knowingly parts from `kubectl`, which keeps such a Pod at `Running` and pushes the truth into a separate `READY 0/1` column: carrying it in the single label is the whole point, since two columns that can disagree are what made the original defect invisible. See ADR 0021.
_Avoid_: Status, phase, health
...
**Uninstall Template**:
A write operation in the Templates Catalog that removes an installed Template from the Cluster by deleting the Template's Namespace, cascading the removal of every namespaced resource the Template created (Deployments, Services, PersistentVolumeClaims, and more) — the same mechanism as Delete Namespace. Deliberately scoped to the Namespace only: it does not remove the component images that Deploy Template's Kaniko Builds pushed to the Cluster's internal Registry, nor any (hypothetical) cluster-scoped resources a Template might create — mirroring Uninstall Operator (leaves CRDs) and Uninstall (Helm) (leaves PVCs), which likewise avoid aggressive cleanup of shared or data-bearing byproducts (see ADR 0017). Requires the user to type the Template's Namespace name in a confirmation dialog before proceeding, reusing the Delete Namespace warning. Single-shot with no rollback, consistent with Deploy Template (ADR 0015). Because Namespace deletion is asynchronous — the Namespace lingers in Terminating, where neither Open Topology nor a fresh Deploy is valid — the card enters a transient disabled "Uninstalling" state on confirmation, then returns on its own to a deployable state once the Namespace is fully gone. Offered only from an already-Installed card.
_Avoid_: Delete Template, remove template, uninstall namespace
...
**Topologia**:
UI view that renders an interactive graph of Kubernetes resources within a Namespace and the relationships between them. Node types: Deployment, StatefulSet, ReplicaSet, Pod, PodGroup, Service, PersistentVolumeClaim, Ingress. Structural edges derived from `ownerReferences` (Deployment→ReplicaSet→Pod, StatefulSet→Pod directly — StatefulSet has no intermediate ReplicaSet), label selector matching (Service→Pod), volume mounts (PodGroup→PersistentVolumeClaim via `spec.volumes[].persistentVolumeClaim.claimName`), and backend service references (Ingress→Service via `spec.rules[].http.paths[].backend.service.name` — only when the target Service exists in the namespace). Also renders inferred `ServiceDependency` edges. Isolated nodes (no edges) are shown — they signal misconfiguration. Pods owned by a Job (directly or via a CronJob's Job) are deliberately excluded — they represent finite task executions, not the long-running service topology this view is meant to map. Clicking a node opens a detail panel; the "Go to resource" button navigates to the resource's listing view pre-filtered by name. Pan and zoom are enabled. Optionally renders `TopologyGroup` containers around nodes sharing `app.kubernetes.io/part-of`/`app.kubernetes.io/component` labels, toggleable via a control that is on by default.
...
**TopologyNode**:
A single resource in the `TopologyGraph`. Carries: a unique `id` (type + name), a display `label` (resource name), a `type` — the kind of thing the node stands for (Deployment, StatefulSet, ReplicaSet, Pod, PodGroup, Service, PersistentVolumeClaim, Ingress), which is what selects the node's icon —, a `subtitle` (the words shown beneath the name: the kind itself for most nodes, the replica count for Pod and PodGroup nodes), a `status` (the word shown to the user), an `alert` (what the node draws on the graph when it needs attention — the ready-against-desired proportion on controller nodes, the `status` word on every other type, and nothing at all when the severity is healthy or neutral, so a healthy graph stays quiet and the one troubled node is the one that speaks), a severity (what colors the node — healthy, neutral, degraded, or problem, decided server-side so the graph never has to recognize Kubernetes vocabulary of its own), and a resource view URL (used by the "Go to resource" button in the detail panel to navigate to the resource's listing view pre-filtered by name). How `status` is derived depends on the node type: controller nodes (Deployment, StatefulSet, ReplicaSet) compare ready against desired replicas, yielding `Running`, `Degraded`, or `Unknown`; a Pod node carries its `PodState`, the same word the Pods listing shows for that Pod; and a synthetic PodGroup node carries the `PodState` of its first troubled Pod by name, falling back to `Running` when none is troubled. A controller node and the Pod group beneath it therefore answer different questions — how many are ready, and why — instead of restating each other, which is why `Degraded` survives on controllers but not on Pod groups, where it named a problem without naming its cause. PersistentVolumeClaim nodes additionally carry capacity, access mode, and storage class. Ingress nodes additionally carry hosts, TLS status ("Secure"/"Plain"), and IngressClass.
_Avoid_: Node, vertex, element

**PodGroup**:
A synthetic `TopologyNode` standing for every Pod owned by the same controller, drawn as a single node rather than one node per replica so the graph stays readable as replica counts grow. Not a Kubernetes resource — it exists only inside `Topologia`. Its label is the base name the replicas share, and its subtitle is the replica count (`3 Pods`). It answers *why* — carrying the `PodState` of its first troubled Pod by name — while the controller node above it answers *how many are ready*; the two deliberately do not restate each other. Pods owned by a Job never form one, being excluded from the graph entirely. "Go to resource" leads to the Pods listing filtered by the group's base name, reaching every replica at once.
_Avoid_: Pod set, replica group, pod cluster

**TopologyEdge**:
A directed relationship in the `TopologyGraph` between two `TopologyNode` ids, carrying a `type` that distinguishes structural relationships (ownership: Deployment/StatefulSet→ReplicaSet/Pod; routing: Service→Pod, Ingress→Service; storage: PodGroup→PersistentVolumeClaim — all derived from fields Kubernetes itself uses to operate the resource) from `ServiceDependency` (an inferred Workload→Service relationship — see ServiceDependency). Structural edges flow from owner/controller to owned, or from Service/Ingress to what they route to.
_Avoid_: Link, connection, arrow

**ServiceDependency**:
A `TopologyEdge` type representing an inferred relationship between a Workload and a Service it appears to depend on — e.g. a backend pointing at its database's Service. Unlike structural edges, it is a heuristic derived from application configuration, not a fact Kubernetes itself enforces. Direction flows from the consuming Workload to the target Service (the reverse of the structural Service→Pod edge). Detected by scanning `env[].value` and `env[].valueFrom` (`configMapKeyRef`/`secretKeyRef`)/`envFrom` across a Pod's `containers` and `initContainers`, resolving ConfigMap/Secret values where needed (these stay invisible in the graph — not rendered as nodes), then matching the resulting string against known Service names in the active Namespace via substring/word-boundary comparison — including the `<service>.<namespace>.svc.cluster.local` pattern, only matched when its namespace segment equals the active Namespace. Multiple matching env vars between the same Workload/Service pair collapse into one edge. Rendered as a dashed, distinctly colored edge; the matched env var and value are shown as evidence in the origin node's detail panel, since the heuristic can produce false positives with short or generic Service names. Does not cover connection strings embedded inside the content of a ConfigMap/Secret mounted as a file — out of scope. Cross-namespace references (a value whose `<service>.<namespace>.svc.cluster.local` segment names a different Namespace) produce no edge — the target Service isn't loaded, and matching it against a same-named local Service would misattribute the dependency. Future direction (not yet implemented): render these as a pointer node to the target Namespace, reusing the "switch active Namespace and navigate to Topologia" affordance already used by Templates Catalog's Open Topology.
...
**Build**:
A write operation that builds an image from a Build Context and pushes it to the Cluster's Registry, under a Repository and Tag chosen by the user. Triggered from the Registry view, from Deploy from Dockerfile, from Import Compose, or from Deploy Template; progress is shown as a live log, similar to PodLog. GreenCap does not persist a history of past Builds — once finished, a Build leaves no trace in GreenCap.
_Avoid_: Deploy, publish

**Build Context**:
What a Build compiles — the Dockerfile plus the files its `COPY`/`ADD` instructions resolve against. Comes from one of two origins: a Git Repository or a Local Folder. The origin is chosen per Build, not configured on the Cluster or the User.
_Avoid_: Source, build source, "context path" (which names only a subdirectory *within* a Build Context)

**Git Repository**:
One of the two origins of a Build Context — a publicly accessible Git repository identified by URL and branch, plus a Context path (the subdirectory used as the build's root, defaults to the repository root) and the path to a Dockerfile within that Context. GreenCap does not store or transfer its contents, only the reference; the Build clones it directly. Only public repositories are supported — private repositories would require credentials, not yet supported. The only origin accepted by Deploy Template (whose repository is curated by the GreenCap team) and by the Registry view's own Build.
_Avoid_: Source code, project, "repository" alone (ambiguous with Repository, the Registry concept)

**Local Folder**:
The other origin of a Build Context — a directory on the User's own computer, chosen through the browser's native folder picker, used for a Build without ever being published to a Git host. Its whole file tree is transferred to the Cluster for that Build and discarded afterwards; GreenCap persists nothing. Entries that never belong in an image (`.git`, `node_modules`, `venv`, `__pycache__`, `target`, `dist`, `build`) are always excluded, and a `.dockerignore` found in the folder excludes further files on top of those — it can only remove more, never re-include what the built-in exclusions removed. Bounded to 50 MB once compressed. Offered by Deploy from Dockerfile and Import Compose only. See ADR 0020.
_Avoid_: Upload, local file, directory, path
...
**Import Compose**:
A wizard-guided write operation that translates a `docker-compose.yml` into Kubernetes resources and provisions them in a single Namespace. One of three deploy modes alongside Deploy Application and Deploy from Dockerfile. The Compose file comes from a Build Context — a Git Repository or a Local Folder — and is always addressed by its path within it, exactly as in Deploy from Dockerfile. Picking the Compose file on its own is deliberately not offered: it would be the only place in the platform where a Build Context is optional, and it would put a wizard step in a state where services declaring `build:` can no longer be built. The wizard runs in three steps: (1) source (Git Repository URL + branch, or a Local Folder) + path to the Compose file within it + Namespace name — GreenCap fetches or reads and parses the file; (2) review screen showing all resources to be created, editable image (for services with `build:`) and PVC fields, the option to expose each service externally, warnings for unsupported or ignored directives, and pending Builds for services with `build:`; (3) execution — Kaniko Builds run first (one per service with `build:`, live log), then all Kubernetes resources are created, and a result summary is shown. Translation rules: each Compose `service` becomes a Deployment; `ports:` becomes a ClusterIP Service on the first declared port; a service that has one can optionally be exposed externally through its own Ingress, off by default and routing to that port — one Ingress per exposed service, never a single Ingress shared through path rules, since a path prefix breaks applications that expect to be served at the root; `environment:` keys matching `PASSWORD`, `SECRET`, `TOKEN`, `KEY`, or `CREDENTIAL` (case-insensitive) go to a Secret (`<service>-secret`), the rest to a ConfigMap (`<service>-config`); named `volumes:` become PersistentVolumeClaims (`<service>-pvc`, defaulting to 1Gi and the cluster's default StorageClass, editable); bind-mount volumes are ignored with a warning; `depends_on:` is ignored with an informative warning; `build:` triggers a Kaniko Build using the same Build Context the Compose file itself came from — each service's `build:` path resolving as a subdirectory within it — pushing to the Cluster's Registry — image name taken from `image:` if declared alongside `build:`, otherwise defaults to `<service-name>:latest` (editable); `image:` without `build:` is used as-is. All generated resources carry `app.kubernetes.io/part-of: <namespace>` and `app.kubernetes.io/component: <service-name>` labels so they appear grouped in Topologia. On full success, navigates to Topologia of the new Namespace; on partial failure, stays on an inline result screen. No rollback on failure.
_Avoid_: Compose deploy, stack import, docker-compose import

**Deploy from Dockerfile**:
A wizard-guided write operation that builds a container image from a Dockerfile in a Build Context via Kaniko, then provisions Kubernetes resources to run that image — combining Build and Deploy Application in a single uninterrupted flow. One of three deploy modes alongside Deploy Application and Import Compose. The wizard runs in six steps: (1) Source + Name — the Build Context origin (a Git Repository, with URL and branch; or a Local Folder), Dockerfile path (defaults to `Dockerfile`), context path (defaults to the Build Context root), and application name (used as the Namespace, auto-suggested from the repository or folder name); (2) Image & Port — image tag auto-suggested as `<namespace>/<namespace>:latest` (editable) and the container port the application listens on; (3) Resources — replica count, CPU request/limit, memory request/limit; (4) Volume (optional) — StorageClass, size in Gi, mount path; (5) External Access (optional) — Ingress host (auto-suggested as `<namespace>.greencap.local`) and IngressClass; (6) Review — summary of all resources to be created. On Review confirmation, a Kaniko Build runs first with inline logs (same polling mechanism as Import Compose); on build success the deployment phase provisions a Namespace, Deployment, and optionally Service/PVC/Ingress using the freshly built image (`localhost:5000/<namespace>/<namespace>:<tag>`). On build or deploy failure, an inline error is shown — no rollback. On full success, navigates to Topologia of the new Namespace.
_Avoid_: Build and deploy, Dockerfile deploy, Docker build

**Tour**:
A one-time guided introduction to the platform, presented to a User on their first session in which a Cluster is active. It dims the screen, spotlights one element of the interface at a time, and explains it in a bubble beside it. Six stops, in the order the eye crosses the screen: the active Cluster and Namespace in the header, Developer Experience, Topologia, Project, Global and Settings, and a closing stop pointing out that what is on screen is GreenCap itself running in that Cluster. Narration only — it never asks the User to perform an action, and the overlay blocks interaction while it runs. Finishing, skipping, or dismissing it all count the same: the Tour does not come back on later logins, and is replayed only on demand from PlatformSettings. The stop that names the Users view appears only for the PlatformAdmin, the only role that view admits. Does not fire when no Cluster is active — the interface would be inert and the Tour would be teaching a screen that cannot respond.
_Avoid_: Onboarding, tutorial, walkthrough, wizard

**TourStep**:
A single stop in a `Tour`: the interface element it spotlights, plus the title and text shown beside it. Ordered, and always shown as a complete sequence — a User cannot pick which stops to see. Decided server-side, including which stops the acting User's role admits; the frontend only renders what it receives.
_Avoid_: Step, tour stop, coach mark

# GreenCap K8s — Guia do Desenvolvedor

Fluxo para quem vai alterar o código-fonte do GreenCap. Para subir a aplicação como usuário final (sem tocar em código), veja o [README principal](../README.md).
...
Requer **Java 21+** e usa o Gradle Wrapper (não precisa instalar Gradle).
...
## Rodando via Docker Compose (build de produção)

Útil para validar o build de produção completo (Vaadin em modo produção) sem provisionar um cluster Kubernetes.

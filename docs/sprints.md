...
#### ✅ ~~Documentação — divulgar `setup/setup.sh` no README principal~~ (resolvido)

> **Resolvido.** O `README.md` hoje apresenta `./setup/setup.sh` como o **Quick Start principal** (não só alternativa ao `docker compose up`): seção "Quick Start" com o comando, o efeito esperado (`http://greencap.local`, login `admin`/`admin` com aviso de troca), o script de teardown (`./setup/teardown.sh`) e a nota de como a imagem é provida (pull público vs. `BUILD_LOCAL`/`PLATFORM_IMAGE_TAG`). Entregue junto da reescrita do Quick Start na Sprint 106.

#### ✅ ~~Divulgação — aprofundar a landing page (`greencapk8s.dev`)~~ (resolvido em jul/2026)

> **Resolvido** (repo do portal `../greencap-k8s-portal`, confirmado 2026-07-21). Redesign completo da landing (PR #3 `Feat/landing page redesign`) com 10 seções — problema, solução, como-funciona, preview, funcionalidades, stack, segurança, diferenciais, público, comunidade; seção de preview mostra o Catálogo de Templates; pipeline de deploy para S3 + CloudFront; homepage do repo `greencap-k8s` já aponta para https://www.greencapk8s.dev/. Fecha o **Tier 3** (último item pendente) da iniciativa de divulgação open source — ver memória `project_oss_first_contact`. Era trabalho no repo do portal, não no `greencap-k8s`.

...
`docs/sprints.md` and `.issue-tracker/` accumulate one entry per sprint indefinitely. To keep both navigable, only the most recent sprints stay in the active files — older ones move to dedicated archives.
...
- **`.issue-tracker/`** — keeps `sprint-N/` directories for the same last 10 sprints. Older directories live under `.issue-tracker/archive/sprint-N/`.
...
3. Move the corresponding `.issue-tracker/sprint-N/` directory to `.issue-tracker/archive/sprint-N/` via `git mv`, preserving its internal structure (`issues/*.md`).
...
Plain markdown, same structure as the active `.issue-tracker/sprint-N/` directories — still greppable, just out of the active working set.
